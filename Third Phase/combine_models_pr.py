...
for name, pipe in models.items():
    ...
    auc_val = average_precision_score(y_test, y_score)
    plt.plot(recall, precision, lw=2, label=f"{name} (AUC = {auc_val:.4f})")

plt.xlabel("Recall")
plt.ylabel("Precision")
plt.title("Combined Precision–Recall Curves (XGBoost, Random Forest, NN)")
plt.legend(loc="lower left")
plt.grid(alpha=0.3)
plt.tight_layout()

save_path = OUTDIR / "combined_models_pr.png"
plt.savefig(save_path, dpi=400)
print(f"Saved combined PR figure → {save_path}")
plt.show()